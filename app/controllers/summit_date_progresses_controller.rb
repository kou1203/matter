class SummitDateProgressesController < ApplicationController
  ...
  def index 
    ...
    if  @billings.present?
      ...
      @billings_low_voltage_data = SummitBillingAmount.where(contract_type: "低圧電力").where("billing_date LIKE ?","%#{@billing_year}%")
  ...
  def base
    ...
    @base = params[:base]
    @month = params[:month] ? Time.parse(params[:month]) : Date.today
    @users_base = @users.where(base: @base).order("position_sub DESC")
    ...
    @billings = SummitBillingAmount.where(billing_date: @year_month).where(base: @base)
    @billings_prev = SummitBillingAmount.where(billing_date: @year_month_prev).where(base: @base)
    @users_data = User.where(base: @base)
    @summit_users = Summit.includes(:user).where(user: {base: @base}).group(:user_id)
    @month_metered_all = 
      SummitBillingAmount.where("contract_type LIKE ?","%従量%").where("billing_date LIKE ?","%#{@billing_year}%")
    @month_low_voltage_all = SummitBillingAmount.where(contract_type: "低圧電力").where("billing_date LIKE ?","%#{@billing_year}%")
    @metered_group = @month_metered_all.group_by {|m| m[:user_id]}
    @month_metered = SummitBillingAmount.where(first_flag: "過去発行済").where("contract_type LIKE ?","%従量%").where("billing_date LIKE ?","%#{@billing_year}%")
    @month_low_voltage = SummitBillingAmount.where(first_flag: "過去発行済").where(contract_type: "低圧電力").where("billing_date LIKE ?","%#{@billing_year}%")
    @users_base = @billings.where(base: @base).includes(:user).group(:user_id).order("user.position_sub DESC")
