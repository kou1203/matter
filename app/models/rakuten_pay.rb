class RakutenPay < ApplicationRecord
  ...
  def self.csv_check(file)
    ...
    CSV.foreach(file.path, headers: true).with_index(1) do |row, index|
      if (row["商材"].nil?) || (row["商材"] != "楽天ペイ")
        errors << "商材が楽天ペイではない可能性があります。CSVの名前を確認してください。"
        break
      end
      ...
      if row["ID"].present?
        ...
        rakuten_pay = new(
          ...
          def_status: row["不備①番号・楽天"],
          def_status2: row["不備②番号・楽天"],
          deficiency: row["不備発生日"],
          deficiency_solution: row["不備解消日"],
          client_def_date: row["上位店不備発生日"],
          client_def_solution:row["上位店不備解消日"],
